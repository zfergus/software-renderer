...
#include <swr/eigen_json.hpp>
#include <swr/raster.hpp>
...
Scene::Scene(const nlohmann::json& scene_json)
{
    ...
    if (scene_json["shading"] == "wireframe") {
        shading = Shading::WIREFRAME;
    } else if (scene_json["shading"] == "flat") {
        shading = Shading::FLAT;
    } else if (scene_json["shading"] == "smooth") {
    ...
    const std::string& filename)
{
    ...
    case Shading::SMOOTH: {
        MatrixXF smooth_normals = MatrixXF::Zero(V.rows(), V.cols());
        Eigen::VectorXi vertex_face_adj_count = Eigen::VectorXi::Zero(V.rows());
        for (int i = 0; i < F.rows(); i++) {
            Vector3F l1 = (V.row(F(i, 1)) - V.row(F(i, 0))).cast<Float>();
            Vector3F l2 = (V.row(F(i, 2)) - V.row(F(i, 0))).cast<Float>();
            Vector3F normal = (l1).cross(l2).normalized();
            for (int j = 0; j < 3; j++) {
                smooth_normals.row(F(i, j)) += normal.transpose();
                vertex_face_adj_count(F(i, j))++;
            }
        }
        for (int i = 0; i < smooth_normals.rows(); i++) {
            smooth_normals.row(i) =
                (smooth_normals.row(i) / vertex_face_adj_count(i)).normalized();
        }
        for (int i = 0; i < F.rows(); i++) {
            for (int j = 0; j < 3; j++) {
                VertexAttributes va(V.row(F(i, j)).cast<Float>());
                va.material = scene.materials[C[F(i, j)]];
                va.normal = smooth_normals.row(F(i, j));
                face_vertex_attributes.push_back(va);
            }
        }
    } break;
    ...
    if (scene.shading != Shading::WIREFRAME) {
        rasterize_triangles(
            create_face_shaders(), uniform, face_vertex_attributes,
            frame_buffer);
    }
